...
class KImage {
	...
	{
		return new KImage($image, $config);
	}
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ( ! isset(KImage::$allowed_types[$image_info[2]]))
		...
		(
			...
			'height' => $image_info[1],
			'type' => $image_info[2],
			'ext' => KImage::$allowed_types[$image_info[2]],
	...
	protected function valid_size($type, & $value)
	{
		...
		switch ($type)
		{
			...
			break;
			case 'master':
				if ($value !== KImage::NONE AND
				    $value !== KImage::AUTO AND
				    $value !== KImage::WIDTH AND
				    $value !== KImage::HEIGHT)
					return FALSE;
			break;
		}
