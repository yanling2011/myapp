...
class Image_GD_Driver extends Image_Driver {
    ...
    public function flip($direction)
	{
		...
		if ($direction === KImage::HORIZONTAL)
		{
			for ($x = 0; $x < $width; $x++)
			{
		...
		elseif ($direction === KImage::VERTICAL)
		{
			for ($y = 0; $y < $height; $y++)
			{
	...
	public function resize($properties)
	{
		...
		if ($properties['master'] === KImage::AUTO)
		{
			// Change an automatic master dim to the correct type
			$properties['master'] = (($width / $properties['width']) > ($height / $properties['height'])) ? KImage::WIDTH : KImage::HEIGHT;
		}

		if (empty($properties['height']) OR $properties['master'] === KImage::WIDTH)
		{
			// Recalculate the height based on the width
			$properties['height'] = round($height * $properties['width'] / $width);
		}

		if (empty($properties['width']) OR $properties['master'] === KImage::HEIGHT)
		{
